        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <div className="flex items-center gap-2 border-l border-secondary/20 pl-6 ml-2">
            <button 
              ...
              className={`text-xs font-black transition-all ${language === 'en' ? 'text-primary scale-110' : (isScrolled ? 'text-text-main/50' : 'text-white/50')}`}
            >EN</button>
            <span className="text-secondary/40 text-xs">|</span>
            <button 
              onClick={() => setLanguage('it')}
              className={`text-xs font-black transition-all ${language === 'it' ? 'text-primary scale-110' : (isScrolled ? 'text-text-main/50' : 'text-white/50')}`}
            >IT</button>
          </div>
        </nav>
        ...
        <div className="md:hidden bg-bg-cream border-b border-secondary/10 absolute w-full top-full left-0 animate-fade-in shadow-2xl overflow-hidden">
          <nav className="flex flex-col px-8 py-10 gap-6">
            ...
            <div className="flex gap-6 border-t border-stone-200 pt-8 mt-4">
               <button onClick={() => {setLanguage('it'); setIsMobileMenuOpen(false);}} className={`text-lg ${language === 'it' ? 'font-black text-primary' : 'text-secondary/60'}`}>Italiano</button>
               <button onClick={() => {setLanguage('en'); setIsMobileMenuOpen(false);}} className={`text-lg ${language === 'en' ? 'font-black text-primary' : 'text-secondary/60'}`}>English</button>
            </div>
            <Link 
              to="/contact" 
              onClick={() => setIsMobileMenuOpen(false)}
